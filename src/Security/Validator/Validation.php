...
*/
use Closure;
use Kiaan\Security\Validator\Rules\Interfaces\BeforeValidate;
use Kiaan\Security\Validator\Rules\Interfaces\ModifyValue;
use Kiaan\Security\Validator\Rules\Required;
use Kiaan\Security\Validator;
...
class Validation
{
    ...
    */
     ...
     * Constructor
     *
     * @param \use Kiaan\Security\Validator $validator
     * @param array $inputs
     * @param array $rules
    ...
    public function __construct(
        ...
        foreach ($rules as $attributeKey => $rules) {
            // Aliases
            $alias = explode(":", $attributeKey)[1] ?? null;
            
            if(!is_null($alias)){
                $this->setAlias($attributeKey, $alias);
            }
     ...
     *
     * @param string $attributeKey
     * @return null|\use Kiaan\Security\Attribute
     */
    ...
    {
        $this->errors = new Errors; // reset errors
     ...
     * Get Errors instance
     *
     * @return \use Kiaan\Security\Validator\Errors
     */
     ...
     * @param \use Kiaan\Security\Attribute $attribute
     * @param mixed $value
     * @param \use Kiaan\Security\Rule $ruleValidator
     * @return void
     */
     ...
     * @param \use Kiaan\Security\Attribute $attribute
     * @param mixed $value
     * @param \use Kiaan\Security\Rule $validator
     * @return mixed
     */
    ...
    protected function resolveRules($rules): array
    {
        ...
        foreach ($rules as $i => $rule) {
            ...
            list($rulename, $params) = array($i, $rule);
            $validator = call_user_func_array($validatorFactory, array_merge([$rulename], $params));
            } else {
     ...
     *
     * @return \use Kiaan\Security\Validator
     */
