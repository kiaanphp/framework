...
|---------------------------------------------------
*/
use Kiaan\Contact\Socket\RFC6455\Handshake\PermessageDeflateOptions;
...
class MessageBuffer {
    ...
    /**
     * @var \Socket\RFC6455\Messaging\Message
     */
    private $messageBuffer;

    /**
     * @var \Socket\RFC6455\Messaging\Frame
     */
    private $frameBuffer;
    ...
    private function processData($data) {
        ...
        if ($this->messageBuffer->isCoalesced()) {
            ...
            if (true !== $msgCheck) {
                $onControl($this->newCloseFrame($msgCheck, 'Socket detected an invalid UTF-8 payload'), $this);
            } else {
     ...
     * @param \Socket\RFC6455\Messaging\FrameInterface|FrameInterface $frame
     * @return \Socket\RFC6455\Messaging\FrameInterface|FrameInterface
     */
    public function frameCheck(FrameInterface $frame) {
        if ((false !== $frame->getRsv1() && !$this->deflateEnabled) ||
            ...
            return $this->newCloseFrame(Frame::CLOSE_PROTOCOL, 'Socket detected an invalid reserve code');
        }

        if ($this->checkForMask && !$frame->isMasked()) {
            return $this->newCloseFrame(Frame::CLOSE_PROTOCOL, 'Socket detected an incorrect frame mask');
        }
        ...
        if ($opcode > 2) {
            if ($frame->getPayloadLength() > 125 || !$frame->isFinal()) {
                return $this->newCloseFrame(Frame::CLOSE_PROTOCOL, 'Socket detected a mismatch between final bit and indicated payload length');
            }
            ...
            switch ($opcode) {
                case Frame::OP_CLOSE:
                    ...
                    if (strlen($bin) === 1) {
                        return $this->newCloseFrame(Frame::CLOSE_PROTOCOL, 'Socket detected an invalid close code');
                    }
                    ...
                    if (!$this->checkUtf8(substr($bin, 2))) {
                        return $this->newCloseFrame(Frame::CLOSE_BAD_PAYLOAD, 'Socket detected an invalid UTF-8 payload in the close reason');
                    }
                ...
                case Frame::OP_PONG:
                    break;
                default:
                    return $this->newCloseFrame(Frame::CLOSE_PROTOCOL, 'Socket detected an invalid OP code');
                    break;
            }
        ...
        if (Frame::OP_CONTINUE === $frame->getOpcode() && 0 === count($this->messageBuffer)) {
            return $this->newCloseFrame(Frame::CLOSE_PROTOCOL, 'Socket detected the first frame of a message was a continue');
        }

        if (count($this->messageBuffer) > 0 && Frame::OP_CONTINUE !== $frame->getOpcode()) {
            return $this->newCloseFrame(Frame::CLOSE_PROTOCOL, 'Socket detected invalid OP code when expecting continue frame');
        }
     ...
     * @param \Socket\RFC6455\Messaging\MessageInterface
    ...
    /**
     * @return \Socket\RFC6455\Messaging\MessageInterface
     */
     ...
     * @return \Socket\RFC6455\Messaging\FrameInterface
     */
